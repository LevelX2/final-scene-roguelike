...
- `npm run check:memory`: UTF-8- und Mojibake-Check für `KI-Wissen-Final Scene`
...
- Bei Änderungen an der Wissensbasis `KI-Wissen-Final Scene` zusätzlich `npm run check:memory` verwenden, damit gemischte Kodierungen oder falsch decodierte Umlaute früh auffallen.
